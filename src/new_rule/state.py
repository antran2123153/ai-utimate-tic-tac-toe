...
class State:
    ...
    def get_valid_moves(self):
        ...
        if len(indices[0]) != 0 and self.global_cells[index_local_board] == 0:
        ...
        ret = []
        for i in range(9):
            if not np.all(self.blocks[i] != 0) and self.global_cells[i] == 0:
...
class State_2(State):
    ...
    def get_valid_moves(self):
        ...
        if (len(indices[0]) != 0) and (not is_occupied) and self.global_cells[index_local_board] == 0:
            self.free_move = False
            return [
                UltimateTTT_Move(
                    index_local_board, index[0], index[1], self.player_to_move
